...
import { BookOpen, Users, StickyNote, FolderOpen, Calendar, ArrowRight, Plus, X, Trash2, Upload, Download } from 'lucide-react';
...
export default function CourseDetailPage() {
  ...
  const [studentForm, setStudentForm] = useState({ name: '', grade: '', gender: '', notes: '' });
  ...
  const addStudentMutation = useMutation({
    mutationFn: async () => {
      ...
      if (existing) {
        ...
        const { data: newStudent, error } = await supabase.from('students').insert({
          user_id: user!.id, name: studentForm.name, grade: studentForm.grade, gender: studentForm.gender, notes: studentForm.notes,
        }).select().single();
        if (error) throw error;
    ...
    onSuccess: () => {
      ...
      setStudentForm({ name: '', grade: '', gender: '', notes: '' });
    },
  ...
  const tabs = [
    ...
    { key: 'schedule' as const, label: t('جدول', 'Schedule'), icon: Calendar, count: sessions.length },
  ];
        ...
        <div className="space-y-4">
            ...
            {showAddStudent && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="glass-card rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <input value={studentForm.notes} onChange={e => setStudentForm(f => ({ ...f, notes: e.target.value }))} placeholder={t('ملاحظات', 'Notes')} className="px-3 py-2 rounded-lg border border-input bg-background text-foreground text-sm" />
                </div>
          ...
          <div className="space-y-2">
            {enrollments.map((enr: any) => {
              ...
              return (
                <div key={enr.id} className="glass-card rounded-xl p-4 flex items-center justify-between">
                  <div>
                    <p className="font-medium text-foreground">{enr.students?.name}</p>
                    <div className="flex items-center gap-2 mt-1">
                      {enr.students?.grade && <span className="text-xs text-muted-foreground">{enr.students.grade}</span>}
                      {payment && (
            ...
            {enrollments.length === 0 && <p className="text-center text-muted-foreground py-8">{t('لا يوجد طلاب في هذه الدورة', 'No students in this course')}</p>}
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          {showAddNote && (
            <div className="glass-card rounded-xl p-4 space-y-3">
              ...
              <textarea value={noteForm.content} onChange={e => setNoteForm(f => ({ ...f, content: e.target.value }))} placeholder={t('المحتوى', 'Content')} rows={4} className="w-full px-3 py-2 rounded-lg border border-input bg-background text-foreground text-sm resize-none" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {notes.map((note: any) => (
              <div key={note.id} className="note-paper rounded-xl p-4 relative group">
          ...
          {sessions.map((s: any) => (
            <div key={s.id} className="glass-card rounded-xl p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{s.session_date} • {s.start_time}{s.end_time ? ` - ${s.end_time}` : ''}</p>
              </div>
            </div>
          ))}
          {sessions.length === 0 && <p className="text-center text-muted-foreground py-8">{t('لا توجد جلسات', 'No sessions')}</p>}
        </div>
      )}
    </motion.div>
  );
